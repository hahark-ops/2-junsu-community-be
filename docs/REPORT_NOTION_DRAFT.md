...
### 1.2.1 왜 EKS를 실제 운영 경로로 바로 선택하지 않았는가

- 이번 프로젝트는 학습/과제 단계에서 **기능 구현, 배포 자동화, 비용 통제**를 동시에 만족해야 했다.
- EKS는 실무적으로 유효한 선택지이지만, 프리티어 계정 기준에서는 클러스터 운영 비용과 주변 리소스(ALB, 노드, 스토리지, 네트워크) 비용이 빠르게 커질 수 있다.
- 따라서 실제 운영 경로는 **단일 EC2 + Docker Compose**로 두고, Kubernetes는 **로컬 Docker Desktop 및 분산 증빙용 K8s 환경**에서만 검증했다.
- 이 방식으로 현재 단계에서는 비용을 통제하면서도, Redis pub/sub 기반의 다중 Pod DM 분산 처리 구조를 학습·증명할 수 있었다.
- 향후 AWS 크레딧이 충분히 확보되면 EKS는 To-Be 아키텍처의 실제 운영 후보로 확장할 수 있다.
...
- 현재 서비스 정책에서 회원 탈퇴는 `soft delete`가 아니라 **hard delete(영구 삭제)** 로 동작한다.
- 사용자가 탈퇴하면 `users` row 자체를 삭제하며, FK `ON DELETE CASCADE`를 통해 아래 데이터도 함께 제거된다.
  - 게시글
  - 댓글
  - 좋아요
  - 세션
  - DM 방/메시지/읽음 상태
  - Web Push 구독 정보
- 이 정책을 선택한 이유는 현재 프로젝트 단계에서 데이터 생명주기를 단순하게 유지하고, 탈퇴 후 **같은 이메일/닉네임으로 재가입 가능**하게 하기 위해서다.
- 따라서 현재 As-Is 기준에서는 “탈퇴 후 복구”보다 “명확한 영구 삭제와 재가입 허용”을 우선한다.
