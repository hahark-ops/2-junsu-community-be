...
추가로, 회원 탈퇴 정책도 정리했습니다.

- 사용자가 탈퇴하면 계정은 비활성 상태로 남는 것이 아니라 **영구 삭제**됩니다.
- 이때 게시글, 댓글, 좋아요, DM 기록, 알림 구독 정보도 함께 정리됩니다.
- 대신 같은 이메일이나 닉네임으로 다시 가입하는 것은 가능하게 만들었습니다.

쉽게 말하면:
- 탈퇴하면 흔적은 남기지 않고 깔끔하게 지우고
- 나중에 같은 사람이라도 새 계정으로 다시 시작할 수 있게 한 것입니다.

## 7. 왜 EKS를 실제 운영 경로로 바로 쓰지 않았는가

EKS는 여러 서버를 체계적으로 운영할 수 있는 좋은 방법입니다.  
다만 이번 프로젝트는 학습과 과제를 진행하는 단계였고, 프리티어 계정에서는 클러스터 자체 비용뿐 아니라 주변 리소스 비용도 빠르게 커질 수 있습니다.

그래서 실제 운영 경로는 비용이 더 낮고 구조가 단순한 **단일 EC2 + Docker Compose**로 유지했습니다.  
대신 Kubernetes는 로컬 환경에서 직접 검증해서, 서버가 여러 개로 나뉘었을 때도 DM이 제대로 전달된다는 점은 별도로 증명했습니다.

즉:
- 실제 운영은 비용을 아끼는 방식으로 선택했고
- 확장 가능한 구조는 로컬 Kubernetes 실험으로 검증한 상태입니다.
